...
class PartnerRequest(models.Model):
	...
	_name = 'partner.demo.request'
	_inherit = ['mail.thread', 'mail.activity.mixin']
	_description = 'Partner Web Request'

	@api.model
	def set_default_demo_request_hostname(self):
		self.env['ir.config_parameter'].sudo().set_param('set_default_demo_request_hostname','192.168.1.14')

	def _get_default_hostname(self):

		return self.env['ir.config_parameter'].sudo().get_param('set_default_demo_request_hostname')

		

	name = fields.Char(string='Name')
	tencongty = fields.Char(string='Tên công ty', track_visibility='onchange')
	nguoilienhe = fields.Char(string='Người liên hệ', track_visibility='onchange')
	sodienthoai = fields.Char(string='Số điện thoại', track_visibility='onchange')
	email = fields.Char(string='Email', track_visibility='onchange')
	masothue = fields.Char(string='Mã số thuế', track_visibility='onchange')
	diachi = fields.Char(string='Địa chỉ', track_visibility='onchange')
	nganhnghe = fields.Char(string='Ngành nghề', track_visibility='onchange')
	quymo = fields.Char(string='Quy mô', track_visibility='onchange')
	phanhe = fields.Char(string='Phân hệ', track_visibility='onchange')
	date_from = fields.Date(string='Ngày bắt đầu', default=fields.Date.context_today)
	date_to = fields.Date(string="Ngày kết thúc",compute='_compute_date_to')
	remaining_days = fields.Integer(string='Ngày còn lại', compute='_compute_remaning_days')
	partner_id = fields.Many2one('res.partner',string="Partner")
	hostname = fields.Char(string="Host Name", default=_get_default_hostname)
	...
	user_ids = fields.One2many('demo.user.data','demo_id')
	...
	user_ad = fields.Char(string="User Admin")
	password_ad = fields.Char(string="Password Admin")
	...
	def _compute_date_to(self):
		temp = 3
		for line in self:
			line.date_to = line.date_from + timedelta(days=temp)


	@api.depends('date_from')
	def _compute_remaning_days(self):
		for line in self:
			temp = 3
			date_today = date.today()
			date_temp = line.date_from + timedelta(days=temp)
			remaining = (date_temp - date_today).days
			if remaining > temp:
				line.remaining_days = temp
			else:
				line.remaining_days = remaining


	@api.onchange('db_id')
	def onchangeDBid(self):
		self.user_ad = self.db_id.user
		self.password_ad = self.db_id.passwd
	...
	def drop_db(self):
		...
		url = "http://"+self.hostname+":"+self.port+"/web/database/drop"
	...
	def next_action(self):
		...
		if self.try_type == 'create':
			...
			self.restore_db()
		elif self.try_type == 'duplicate':
			self.duplicate_db()
		elif self.try_type == False:
			raise ValidationError("Phải chọn Type để tiếp tục")
	...
	def send_email(self):
		self.ensure_one()
		record_id = self.env['ir.model.data'].get_object_reference('ex_request_try_demo','email_template_user_account')[1]
	...
	@api.model
	def schedule_send_remaining(self):
		
		for line in self.search([('remaining_days','=',0)]):
			line.drop_db()
			record_id = self.env['ir.model.data'].get_object_reference('ex_request_try_demo',
			'email_template_remaining_days')[1]
			template_id = self.env['mail.template'].browse(record_id)
			#raise Warning(template_id)
...
class UserData(models.Model):
	_name = "demo.user.data"
	_description = "User Data"

	demo_id = fields.Many2one('partner.demo.request')
	...
	def create_user(self):
		...
		if self.demo_id.try_type == 'create':
			Connection = XMLRPC(host_usr, self.demo_id.port, self.demo_id.db_name, self.demo_id.email, self.demo_id.passwd)
		elif self.demo_id.try_type == 'duplicate':
			Connection = XMLRPC(host_usr, self.demo_id.port, self.demo_id.new_db_name, self.demo_id.email, self.demo_id.passwd)
		else:
			Connection = XMLRPC(host_usr, self.demo_id.port, self.demo_id.db_name, self.demo_id.user_ad, self.demo_id.password_ad)
